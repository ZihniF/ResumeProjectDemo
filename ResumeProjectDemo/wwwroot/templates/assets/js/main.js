...
(function($) {
	...
	function malat_spyscroll() {
		...
		window.addEventListener('scroll', function () {
			if (smoothScrollRun === true) return;

			var scrollPosition = window.scrollY || document.documentElement.scrollTop;

			var sections = getSectionsOffsets(); // her scrollda güncel offset alıyoruz

			sections.forEach(function (section) {
				if (scrollPosition >= section.offsetTop) {
					if ($('#navigation a[href="#' + section.id + '"]').length) {
						$('#navigation .active').removeClass('active');
						$('#navigation a[href="#' + section.id + '"]').addClass('active');
					}
				}
			});
